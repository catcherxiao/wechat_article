...
## 更新摘要
**变更内容**   
- 新增状态端点(/api/status)和健康检查功能的详细说明
- 更新服务器部署方法，包含新的环境变量配置
- 添加访问令牌认证机制的完整说明
- 更新部署后验证方法，强调健康检查的重要性
- 整合从README_DEPLOY.md和VERCEL_GUIDE.md转移过来的详细部署说明
...
**更新** 新增了状态端点(/api/status)和健康检查功能，以及更详细的访问令牌认证机制说明。
...
**更新** 新增了访问令牌认证机制，支持多种认证方式（请求头、Bearer Token、请求体参数）。
...
**更新** 新增了状态端点和健康检查功能的重要性说明，建议在所有部署环境中都配置并定期检查健康状态。
...
**更新** 强调了健康检查的重要性，建议在所有部署环境中都进行验证。
...
### 访问令牌认证机制
- 支持的认证方式
  - 请求头：X-Article-Jike-Access-Token
  - Bearer Token：Authorization: Bearer <token>
  - 请求体参数：accessToken 字段
- 环境变量配置
...
### 服务器 systemd 部署
- 最小 systemd 服务配置
  - Unit 配置：描述、网络依赖、启动目标
  - Service 配置：用户权限、工作目录、环境文件、启动命令、重启策略
  - Install 配置：多用户目标
- 部署检查
  - JavaScript 语法检查
  - 系统服务管理器重载
...
**新增** 完整的服务器 systemd 部署配置说明。

**章节来源**
- [README_DEPLOY.md:92-126](file://README_DEPLOY.md#L92-L126)
